...
export default {
  ...
  methods: {
    ...
    next(departmentId) {
      this.deep += 1;
      this.$router.push({ name: 'tickets-id-principal-user-selector-department-id', params: { id: this.$route.params.id, departmentId } });
      this.selectedUserId = null;
    },
    async confirm() {
      ...
      try {
        this.isLoading = true;
        await this.$axios.post('/api/malfunctionReporting/designatePrincipal', {
          uuid: this.$route.params.id,
          principalUserID: this.selectedUserId,
          principalUserApartmentID: `${this.currentDepartment.id}`,
        });
