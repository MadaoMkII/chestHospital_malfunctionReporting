...
export default {
  ...
  methods: {
    ...
    next(departmentId) {
      this.deep += 1;
      this.$router.push({ name: 'tickets-id-cooperation-user-selector-department-id', params: { id: this.$route.params.id, departmentId } });
    },
    async confirm() {
      ...
      try {
        this.isLoading = true;
        await this.$axios.post('/api/malfunctionReporting/designateAssistant', {
